...
#' @export
set_timeseries_frequency <- function(lemdt_result, freq=.25, ref_arena=0){
  ...
  lemdt_result2 <- lemdt_result[, t_round := floor(t/ freq) * freq]
    
  lemdt_result <- lemdt_result2 %>% group_by(arena, t_round) %>%
    summarise(
