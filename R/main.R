...
##############################################
Destination <- Sys.getenv("IDOC_RESULTS_TEMP")
setwd(Destination)
...
lapply(1:9, function(border_mm) {
  ...
  #################### creat new folder for results ##################
  experiment_folder_01_Paired_1 = experiment_folder
  path <- experiment_folder_01_Paired_1
  ...
  PLOTS_PI_folder = paste0(Destination,"/",experiment_folder_01_Paired_1,"/",newfolder)
