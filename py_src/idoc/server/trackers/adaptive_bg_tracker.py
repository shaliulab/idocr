...
class ObjectModel(Root):
    ...
    def distance(self, features, time):
        if time - self._last_updated_time > self._max_unupdated_duration:
            ...
            self.__init__(history_length=self._history_length)
            return 0
...
class BackgroundModel(Root):
    """
    A class to model background. It uses a dynamic running average and support arbitrary and heterogeneous frame rates
    """
    def __init__(self, *args, max_half_life=500. * 1000, min_half_life=5.* 1000, increment=1.2, **kwargs):
...
class AdaptiveBGModel(BaseTracker, Root):
