...
while True:
	...
	_, frame1 = cv2.threshold(frame, 140, 255, cv2.THRESH_BINARY_INV)
	...
	if True:
		...
		(im3, contours1, hierarchy1) = cv2.findContours(frame1, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) 
		for c in contours1:
			if cv2.contourArea(c) < 90 or cv2.contourArea(c)>200:
				continue
			cnt = c
			M = cv2.moments(cnt)
			#print(M)
			...
			cv2.putText(frame1, "FPS: " +str(VIDEO_FPS), (50,40), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (50,100,50), 2)
			cv2.putText(frame1, "X: " +str(cx1), (50,40), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (50,170,50), 2)
			cv2.putText(frame1, "Y: " +str(cy1), (50,60), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (50,170,50), 2)
